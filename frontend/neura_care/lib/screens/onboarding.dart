...
class _OnboardingScreenState extends ConsumerState<OnboardingScreen> {
  ...
  Future<void> submitAnswers(String answer) async{
    setState(() {
      answers[currentQuestionIndex] = answer;
    });
    if(!await InternetConnection().hasInternetAccess){
      if(mounted){
        ...
        if (messenger != null) {
          messenger.showSnackBar(const SnackBar(content: Text('No internet connection')));
        }
      }
      return;
    }
  ...
  await createUserVitals(token!, vitals);
  final score = await getScore(token);
      ...
      print('Error submitting vitals: $e');
        if(mounted){
          final messenger = ScaffoldMessenger.maybeOf(context);
          if (messenger != null) {
            messenger.showSnackBar(SnackBar(content: Text('Error submitting vitals: $e')));
          }
        }
    }
  }
